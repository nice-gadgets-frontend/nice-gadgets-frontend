...
export function Dropdowns({
  ...
  return (
    <Dropdown>
      <DropdownTrigger>
        <Button variant="bordered" className={buttonClassName + " flex justify-between items-center px-3"} disableRipple>
          <span>{selectedLabel}</span>
          <ChevronDown size={16} className="text-[#F1F2F9]" />
        </Button>
      </DropdownTrigger>
      ...
      <DropdownMenu
        ...
        itemClasses={{
          ...
          wrapper: "data-[selected=true]:bg-[#323542] data-[selected=true]:text-[#F1F2F9] bg-transparent",
          selectedIcon: "text-[#F1F2F9]",
        }}
        onSelectionChange={(keys) => onChange(new Set(keys as Iterable<string>))}
      >
        {options.map((option) => (
