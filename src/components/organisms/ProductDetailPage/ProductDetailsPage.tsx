...
<main className="max-w-6xl mx-auto font-[mont] text-primary">
  {/* breadcrumb */}
  <div className="flex items-center text-sm text-secondary mb-8 gap-2">
    ...
    <Link to={`/${category}`} className="text-primary capitalize">
      {category}
    </Link>
  ...
  <h1 className="text-3xl font-mont mb-6">{product.name}</h1>

  {/* зображення + деталі */}
  <section className="max-w-[900px] ml-0 flex flex-col md:flex-row gap-16 mb-16">
    <div className="md:flex-1">
      <SwiperGallery images={product.images ?? []} altPrefix={product.name} />
    </div>

    <div className="flex flex-col gap-6 max-w-[320px] md:flex-[0.45]">
      {/* кольори */}
  ...
  {/* Опис */}
  <section className="grid grid-cols-1 lg:grid-cols-[2fr_1fr] gap-10 mt-10">
    <section>
      <div className="mb-6">
        <h2 className="border-b border-b-[0.5px] border-elements pb-3 text-2xl font-bold">About</h2>
      </div>
      {product.description.map((section, idx) => (
        <article key={idx} className="mb-4">
          <h3 className="text-lg font-semibold mb-1">{section.title}</h3>
          {section.text.map((paragraph, pidx) => (
            <p key={pidx} className="leading-relaxed text-secondary mb-1">
              {paragraph}
            </p>
          ))}
        </article>
    ...
    <section>
      <div className="mb-6">
        <h2 className="border-b border-b-[0.5px] border-elements pb-3 text-2xl font-bold">Tech specs</h2>
      </div>
